...
import com.android.scy.pictureclass.AddLabelActivity;
 ...
 */
public class TagCloudViewMe extends ViewGroup {

    private static final String TAG = me.next.tagview.TagCloudView.class.getSimpleName();
    private static final int TYPE_TEXT_NORMAL = 1;
    private List<LabelModel> tags;

    private LayoutInflater mInflater;
    private TagCloudViewMe.OnTagClickListener onTagClickListener;
    private TagCloudViewMe.OnLongTagClickListener onLongTagClickListener;
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = me.next.tagview.R.drawable.tag_background;
    ...
    private static final int DEFAULT_TAG_RESID = me.next.tagview.R.layout.item_tag;
    private static final int DEFAULT_RIGHT_IMAGE = me.next.tagview.R.drawable.arrow_right;
    ...
    public TagCloudViewMe(Context context) {
        this(context, null);
    }

    public TagCloudViewMe(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TagCloudViewMe(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                me.next.tagview.R.styleable.TagCloudView,
                defStyleAttr,
                defStyleAttr
        );

        mTagSize = a.getDimension(me.next.tagview.R.styleable.TagCloudView_tcvTextSize, DEFAULT_TEXT_SIZE);
        mTagColor = a.getColor(me.next.tagview.R.styleable.TagCloudView_tcvTextColor, DEFAULT_TEXT_COLOR);
        mBackground = a.getResourceId(me.next.tagview.R.styleable.TagCloudView_tcvBackground, DEFAULT_TEXT_BACKGROUND);
        mViewBorder = a.getDimensionPixelSize(me.next.tagview.R.styleable.TagCloudView_tcvBorder, DEFAULT_VIEW_BORDER);
        mTagBorderHor = a.getDimensionPixelSize(
                me.next.tagview.R.styleable.TagCloudView_tcvItemBorderHorizontal, DEFAULT_TEXT_BORDER_HORIZONTAL);
        mTagBorderVer = a.getDimensionPixelSize(
                me.next.tagview.R.styleable.TagCloudView_tcvItemBorderVertical, DEFAULT_TEXT_BORDER_VERTICAL);
        mCanTagClick = a.getBoolean(me.next.tagview.R.styleable.TagCloudView_tcvCanTagClick, DEFAULT_CAN_TAG_CLICK);

        mRightImageResId = a.getResourceId(me.next.tagview.R.styleable.TagCloudView_tcvRightResId, DEFAULT_RIGHT_IMAGE);
        mSingleLine = a.getBoolean(me.next.tagview.R.styleable.TagCloudView_tcvSingleLine, DEFAULT_SINGLE_LINE);
        mShowRightImage = a.getBoolean(me.next.tagview.R.styleable.TagCloudView_tcvShowRightImg, DEFAULT_SHOW_RIGHT_IMAGE);
        mShowEndText = a.getBoolean(me.next.tagview.R.styleable.TagCloudView_tcvShowEndText, DEFAULT_SHOW_END_TEXT);
        endTextString = a.getString(me.next.tagview.R.styleable.TagCloudView_tcvEndText);

        mTagResId = a.getResourceId(me.next.tagview.R.styleable.TagCloudView_tcvTagResId, DEFAULT_TAG_RESID);
            ...
            endText.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    if (onTagClickListener != null) {
                        onTagClickListener.onTagClick(new LabelModel(),view);
                    }
                }
            });
        }
    ...
    public void setTags(List<LabelModel> tagList) {
        this.tags = tagList;
        this.removeAllViews();
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (mTagResId == DEFAULT_TAG_RESID) {
                    tagView.setBackground(getContext().getResources().getDrawable(R.drawable.labelsadd));
                ...
                final LabelModel finalI = tags.get(i);
                tagView.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (onTagClickListener != null) {
                            onTagClickListener.onTagClick(finalI,v);
                        }
                    }
                });
                tagView.setOnLongClickListener(new OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        if(onLongTagClickListener != null){
                            onLongTagClickListener.onLongTagClick(finalI,v);
                        }
                        return false;
                    }
                });
    ...
    public void addTag(final LabelModel labelModel){
        TextView tagView = (TextView) mInflater.inflate(mTagResId, null);
        if (mTagResId == DEFAULT_TAG_RESID) {
            tagView.setBackground(getContext().getResources().getDrawable(R.drawable.labelsapply));
            tagView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTagSize);
            tagView.setTextColor(mTagColor);
        }
        LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        tagView.setLayoutParams(layoutParams);
        tagView.setText(labelModel.getName());
        tagView.setPadding(25,20,25,20);
        tagView.setTag(TYPE_TEXT_NORMAL);
        tagView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onTagClickListener != null) {
                    onTagClickListener.onTagClick(labelModel,v);
                }
            }
        });
        tagView.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if(onLongTagClickListener != null){
                    onLongTagClickListener.onLongTagClick(labelModel,v);
                }
                return false;
            }
        });
...
//        requestLayout();
    ...
    public void setOnTagClickListener(AddLabelActivity onTagClickListener) {
        this.onTagClickListener = onTagClickListener;
    }

    public void setOnLongTagClickListener(AddLabelActivity onLongTagClickListener) {
        this.onLongTagClickListener = onLongTagClickListener;
    }



    public interface OnTagClickListener{
        void onTagClick(LabelModel label,View view);
    }

    public interface OnLongTagClickListener{
        void onLongTagClick(LabelModel label,View view);
    }
